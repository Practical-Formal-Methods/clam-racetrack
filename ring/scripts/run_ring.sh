...
pushd "$SRC_DIR"
	...
	do
		...
		do
			x=`expr $px + 1`
			y=`expr $py + 1`
			map_pos="$(cat ./scripts/ring.csv | head -n $x | tail -n 1 | cut -d',' -f$y)"
			if [[ $map_pos == "x" ]]; then
				echo "x" > "$RESULT_DIR/main_${px}_${py}.log"
			elif [[ $map_pos == "g" ]]; then
				echo "g" > "$RESULT_DIR/main_${px}_${py}.log"
			else
				sed "s/Position start_position = {0, 14};/Position start_position = {$px, $py};/g" ./scripts/ring.c > "$TMP_DIR/main_${px}_${py}.c"
				if [[ $1 == "noise" ]]; then
	...
	pushd $SCRIPT_DIR
		...
		date -d@$(echo "scale=2; (`grep 'User time' $SRC_DIR/$TMP_DIR/stats.log | rev | cut -d' ' -f1 | rev` + `grep 'System time' $SRC_DIR/$TMP_DIR/stats.log | rev | cut -d' ' -f1 | rev`) / 695" | bc -l) -u +%M:%S >> $SRC_DIR/$RESULT_DIR/stats.log
