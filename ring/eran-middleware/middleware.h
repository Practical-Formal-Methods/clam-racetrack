...
public:
    ...
    static const int map_height = 45;
    ...
    static std::vector<int> run(std::string network, darguments input_box,std::string JSON) {
        ...
        std::pair<int, relation> graph = robust_rlv(BOX, CUSTOMBOX, network,
                                                    "", 0,
                                                    input_box,
                                                    true, "./bound.log", "./summary.txt",
                                                    dump, JSON);
        return maximum_of(graph);
    }

    static std::vector<int> run_eran(std::string python_exec, std::string eran_path, std::string input_box_path,
                                     std::string relation_diagram_path,std::string output_bound_path,
                                     std::string network, darguments input_box) {

        write_box_to_file(input_box, input_box_path);

        std::stringstream ss;
        ss << python_exec << " " << eran_path << " "
           << "--netname" << " " << network << " "
           << "--input_box_cons" << " " << input_box_path << " "
           << "--relation_diagram" << " " << relation_diagram_path << " "
           << "--domain deeppoly" << " "
           << "--dataset race-track" << " "
           << "--output_bound" << " " << output_bound_path
           << " 2>/dev/null";
        //std::cout << ss.str() << std::endl;
        system(ss.str().c_str());

        auto graph = read_graph_from(relation_diagram_path);
