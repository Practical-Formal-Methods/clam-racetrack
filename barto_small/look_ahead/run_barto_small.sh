...
if grep -q "int noise = 1" /clam/crab/include/crab/analysis/abs_transformer.hpp; then
	if [ -z "$1" ]
	then
		echo "Controller Noise is turned on. parameter noise needs to be passed"
		exit 0
	fi
	if [[ "$1" != "noise" ]]; then
...
pushd "$SRC_DIR"
	...
	do
		...
		do
			if [[ $px -ge 0 && $px -le 31 && $py -ge 0 && $py -le 4 ]]; then
				...
				sed "s/Position start_position = {0, 14};/Position start_position = {$px, $py};/g" look_ahead/barto_small.c > "$TMP_DIR/main_${px}_${py}.c"
				if [[ $1 == "noise" ]]; then
	...
	pushd $SCRIPT_DIR
		...
		echo -n "Avg Time (h:mm:ss): " >> $SRC_DIR/$RESULT_DIR/stats.log	
		date -d@$(echo "scale=2; (`grep 'User time' $SRC_DIR/$TMP_DIR/stats.log | rev | cut -d' ' -f1 | rev` + `grep 'System time' $SRC_DIR/$TMP_DIR/stats.log | rev | cut -d' ' -f1 | rev`) / 233" | bc -l) -u +%H:%M:%S >> $SRC_DIR/$RESULT_DIR/stats.log
