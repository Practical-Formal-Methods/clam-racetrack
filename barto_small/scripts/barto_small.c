...
Position get_start_position(const Map map)
{
    ...
    //Position start_position = {__CRAB_nd(), __CRAB_nd()};
    //__CRAB_assume(start_position.x >= 33);
    //__CRAB_assume(start_position.x <= 34);
    //__CRAB_assume(start_position.y >= 10);
    //__CRAB_assume(start_position.x >= 11);
...
Map get_map()
{
    Map map = {35, 12, 3, 4}; //For our fixed Map
